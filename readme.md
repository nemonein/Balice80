...
- 원활한 한영전환을 위해선 KDE 설정/fcitx5 설정이 필요하다.
...
저 키들(KR_HAEN 등등)은 주 레이어가 아닌 다른 레이어에 정의되어 있고, 실제 키를 눌러도 기능이 작동하지만, 그보다는 코드에서 직접 발사(!)하게끔 설계했다.
다시 말해서, `Shift+Space`를 누르면 QMK는 層을 바꾼 뒤, `register_code` 또는 `tap_code16`를 통해 `KR_HAEN` 등도 출력한다. 이 키 입력을 받은 KDE 는, 단축키 설정에 따라 `fcitx5-remote -s hangul` 명령등을 수행하고, 결국 입력기 자판 전환이 된다.

\*\* 위 그림에 빠져있는데, **동작방식**에서 다음 두가지를 선택해줘야 한다.

- Reset state on Focus in : 아니오
- 입력 상태 공유 : 아니오

위 기능은 각 프로그램별로 입력기 상태를 정해줄 수 있게 한다. 예를 들어, FF 에선 한글 상태에 있다가, 터미널로 이동하면 초기화되어 영어로 돌아가고, 또 다시 FF 로 가면 이전 상태를 기억해서 한글로 설정되게 할 수 있다.
그러나.. QMK 로 하드웨어 Dvorak 자판을 구현했기 때문에, 이 기능을 그대로 쓰면 '매뎌'상태가 나오기도 한다.
이에 좀 더 현명하게(?) 대처하고자, fcitx5-lua 의 힘을 빌렸다.

---

# Fcitx-lua 사용

프로그램이 바뀔 때, 자판 설정을 유지하고, 새 프로그램에서는 무조건 영어로 기본 자판을 띄워주기 위해서 Fcitx-lua 가 필수다.
설치하고, `~/.local/share/fcitx5/lua/imeapi/extensions/to_dvorak.lua` 등을 넣어준다.
또, 이 기능을 위해선 [QMK Raw HID](https://docs.qmk.fm/features/rawhid)를 구현해야 한다.
