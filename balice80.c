...
layer_state_t layer_state_set_user(layer_state_t state) {
  if (is_keyboard_master()) {
    // bool ledState = get_highest_layer(state) > 0; // 원래값. Layer 값이 1
    // 이상이면 불이 들어온다.
    bool ledState =
        get_highest_layer(state) >
        1; // 2 이상일 때만 들어오게끔 바꿨다.
           // 다시 말하면, _BADV, _BADQ 에선 불이 안들어오고, 그 상위일 때만
           // 들어온다. 이렇게 하고 나니, 왼 Ctrl/Alt/GUI 키를 눌렀을 때 불이
           // 안들어온다. 또, 한/영 전환할 때도 불이 안들어온다. layer_move 로
           // 인해 불이 들어왔었다.
    // bool ledState = (layer == 2 || layer == 3);
    // 또는, 위와 같이 특정 레이어일 때만 불이 들어오게 할 수도 있다.
    if (is_keyboard_left()) {
...
void user_sync_layer_led_handler(uint8_t in_buflen, const void *in_data,
