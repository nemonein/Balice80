#!/usr/bin/env lua

-- 2026.04.21
-- 이전 프로그램과 현재 프로그램이 다를 때만 작동하게끔.
-- 또, 이전 프로그램 입력기 상태를 기억했다가 그 입력기를 다시 켜주게끔 설정.
-- 구글 AI 로... (놀라운 세상..)
-- 거의 완벽하게 동작..
--   영어일 땐 Dvorak
--   한글일 땐 Qwerty
-- 위 동작만 하게끔 바꾸면 될 듯...?

local fcitx = require("fcitx")

-- 프로그램별 마지막 입력기를 저장할 테이블
local program_im_history = {}
-- 상태를 유지하기 위한 변수 (스크립트 실행 동안 유지됨)
local last_program = "None"
-- 예외.. albert 와 krunner
local exceptions = { albert = true, krunner = true }

local DEFAULT_IM = "keyboard-us"
local KOR_IM = "hangul"
local JAP_IM = "mozc"
local saved_im = DEFAULT_IM

function on_focus_out()
	local p_name = fcitx.currentProgram() or "Unknown"
	-- 포커스가 나갈 때 해당 프로그램이 쓰던 입력기를 저장
	program_im_history[p_name] = fcitx.currentInputMethod()
	last_program = p_name
end

-- 포커스 이벤트 핸들러
-- local function handle_focus()
function on_focus_in()
	local current_program = fcitx.currentProgram() or "Unknown"
	-- 이전에 기록된 입력기가 있다면 가져오고, 없으면 기본값 사용
	-- fcitx.setCurrentInputMethod(saved_im)
	-- 프로그램이 변경되었는지 확인
	if exceptions[current_program] then
		saved_im = DEFAULT_IM
	elseif current_program ~= last_program then
		saved_im = program_im_history[current_program] or DEFAULT_IM
	end
	-- 입력기를 기본값으로 설정
	fcitx.setCurrentInputMethod(saved_im)
	-- fcitx.setCurrentInputMethod(DEFAULT_IM)
	local log_msg =
		string.format("[%s] Focus Changed: %s -> %s", os.date("%Y-%m-%d %H:%M:%S"), last_program, current_program)

	-- if 構文을 아래 테이블로.
	-- 초보자인 나는 if 를 썼으나, 전문가인 AI 는 테이블을..
	--[[ local toQMK = 1 -- 기본값은 1 (영어)
		if saved_im == DEFAULT_IM then
			toQMK = 1
		elseif saved_im == KOR_IM then
			toQMK = 2
		elseif saved_im == JAP_IM then
			toQMK = 3
		end ]]

	-- 입력기별 QMK 값 매핑 테이블
	local im_to_qmk_map = {
		[DEFAULT_IM] = 1,
		[KOR_IM] = 2,
		[JAP_IM] = 3,
	}

	-- local toQMK = 1 -- 기본값은 1 (영어)
	local toQMK = im_to_qmk_map[saved_im] or 1

	-- 로그 파일에 기록 (백그라운드 실행)
	log_msg = log_msg .. "\n" .. saved_im
	local cmd_log = string.format('echo "%s" >> /tmp/fcitx-focus-test-comparing-prgs.txt &', log_msg)
	local cmd_qmk = string.format("%s/bin/qmk_change_to_dvorak.sh %s", os.getenv("HOME"), toQMK)
	-- cmd_log = cmd_log .. "\n" .. saved_im
	os.execute(cmd_log)
	os.execute(cmd_qmk)
	-- end
end

fcitx.watchEvent(fcitx.EventType.FocusOut, "on_focus_out")
fcitx.watchEvent(fcitx.EventType.FocusIn, "on_focus_in")
